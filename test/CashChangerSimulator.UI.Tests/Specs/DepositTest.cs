...
public class DepositTest : IDisposable
{
    ...
    private Window OpenDepositTerminal(Window mainWindow)
    {
        ...
        var depositWindow = UiTestRetry.FindWindow(_app.Application, _app.Automation, "DepositWindow", TimeSpan.FromSeconds(15));
    ...
    private void FillBulkInsert(Window depositWindow, string quantity)
    {
        ...
        var firstTextBox = UiTestRetry.Find(() => dialog.FindFirstDescendant(cf => cf.ByAutomationId("BulkQuantityBox"))?.AsTextBox(), TimeSpan.FromSeconds(10)) as TextBox;
